...
<div class="{{ $modalClass }}" role="document">
      <div class="modal-content">
        ...
        @if(view()->exists('vendor.starmoozie.crud.fields.relationship.form_content'))
            @include('vendor.starmoozie.crud.fields.relationship.form_content', [ 'fields' => $fields, 'action' => $action])
        @else
        ...
        <button type="button" class="btn btn-secondary" id="cancelButton">{{trans('starmoozie::crud.cancel')}}</button>
          <button type="button" class="btn btn-primary" id="saveButton">{{trans('starmoozie::crud.save')}}</button>
        </div>
      </div>
    </div>
  </div>
